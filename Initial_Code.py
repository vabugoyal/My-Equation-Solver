...
save_repetition = []
while(True):
    def finding_reduced_form():
        from fractions import Fraction
    #   asking for the type of operations
        print("* Enter 'a' if you want to find the solutions to a set of equations")
        print("  like for systems of the form Ax = b")
        print("--------------------------------------------------------------------------")
        print("* Enter 'n' if you want to find any of the following.")
        print("a. The rank of the coefficient matrix.")
        print("b. The column space of the  coefficient matrix.")
        print("c. The row space of the  coefficient matrix.")
        print("d. The nullspace of the coefficient matrix.")
        print("e. The left nullspace of the coefficient matrix.")
        print("--------------------------------------------------------------------------")
        print("* You will also get the E matrix corresponding to the given matrix.")
        print("* An E matrix is the matrix which when multiplied by the intial matrix")
        print("  produces the 'rref' of the intially entered matrix.")
        print("--------------------------------------------------------------------------")
        while(True):
            typ = input("Enter here: ")
            print("--------------------------------------------------------------------------")
            if typ.lower()=='n' or typ.lower()=='a':
                break
            else:
    ...
    #   getting the dimensions of the matrix
        if typ.lower()=='n':
            while(True):
                n = input("Enter the no. of rows in the matrix: ")
                try:
                    n = int(n)
                    break
                except ValueError:
            ...
            while(True):
                n1 = input("Enter the no. of columns in the matrix: ")
                try:
                    n1 = int(n1)
                    break
                except ValueError:
            ...
            print("* You have to consider the augmented of the system Ax=b")
            while(True):
                n = input("Enter the no. of rows in the matrix: ")
                try:
                    n = int(n)
    ...
    #   giving some instructions
        ...
        print("* For example for row1 of some 3X5 augmented matrix, type: 5 6 7 3 0")
        print("--------------------------------------------------------------------------")
            ...
            while(True):
                ...
                try:
                    ...
                    if len(x)==n1:
                        ...
                        print(f"You have to enter {n1} numbers.")
                        continue
                except:
                    print("Read the instructions and enter again!")
                    continue
    #   getting the inital matrix
        inital_matrix = []
        for i in range(len(given_matrix)):
            temp = []
            for j in range(len(given_matrix[i])):
                temp.append(str(given_matrix[i][j]))
            inital_matrix.append(temp)
    ...
    #   here r is the rank of the matrix and n is the number of columns vectors
        ...
        #    printing the row space of the given matrix::::::::
            ...
            rhs_ls = [f"{indp[i]}{row_basis[i]} + " for i in range(len(row_basis)-1)]
            for i in rhs_ls:
                print(i, end="")

            print(f"{indp[-1]}{row_basis[-1]}")
            print("   ", end="")
        ...
        #    printing the column space of the given matrix
            ...
            for i in pivot_col:
                temp = []
                for j in range(n):
                    element = inital_matrix[j][i]
                    temp.append(element)
                col_basis.append(temp)

            indp = [f"b{i+1}" for i in range(len(col_basis))]
            rhs_ls = [f"{indp[i]}{col_basis[i]} + " for i in range(len(col_basis)-1)]
            for i in rhs_ls:
                print(i, end="")

            print(f"{indp[-1]}{col_basis[-1]}")
            print("   ", end="")
        ...
        #   printing the left nullspace of the given matrix
            ...
            if n-local_rank==0:
                ...
                rhs_ls = [f"{indp[i]}{lft_nul_basis[i]} + " for i in range(len(lft_nul_basis)-1)]
                for i in rhs_ls:
                    print(i, end="")
                print(f"{indp[-1]}{lft_nul_basis[-1]}")
                print("   ", end="")
    ...
    #   when the rows>=columns(unknowns)
        if typ == 'a' and n >= n1-1:
                if n1-1 == rank: # there is definitely trivial solution
                    ...
                    print("2. The free variables are:", free)
                    i = 0
                    ...
                    for i in range(len(new_solution_matrix) - 1):
                        print("   ",new_solution_matrix[i])

                    print("4. The pointing vector to the solution space is:")
                    print("   ", new_solution_matrix[-1])

                    print("5. Hence, the complete solution is following: ")
                    print("(*) ", end="")
                    variables_list = [f'x{i + 1}' for i in range(n1 - 2)]
                    rhs_ls = [f" + {free[i]}{new_solution_matrix[i]}" for i in range(len(new_solution_matrix) - 1)]
                    print(f"{variables_list} = {new_solution_matrix[-1]}", end="")
                    for i in rhs_ls:
                        print(i, end="")
                    print()
